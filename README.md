...
<img src="docs/icon.png" alt="The icon" width="128" align="right">

The application icon is drawn the same way — the panel's VU meter in its walnut
case, from the same palette and the same primitives, so it cannot drift away from
what the plug-in actually looks like. `Source/gui/Icon.cpp` renders it at any
size, dropping detail as it shrinks: the numerals go first, then the lettering,
then the fine graduations, and every stroke has a floor of one whole pixel so the
16 px version stays a legible instrument instead of a grey smudge. The two PNGs
CMake hands to JUCE are generated by the dev tool, not maintained by hand.
...
build/KilocycleDevTool_artefacts/Release/KilocycleDevTool icon docs/icon.png 1024
build/KilocycleDevTool_artefacts/Release/KilocycleDevTool icon docs/icon-small.png 128
```
...
window (an optional third argument picks the valve), and `icon` does the same for
the application icon at whatever size you ask for — which makes all three usable
in CI. The two committed icon PNGs are what CMake feeds to JUCE; regenerate them
with the commands above after changing `Source/gui/Icon.cpp`.
